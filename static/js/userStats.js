...
const updateTopMonthsUI = (topMonth, type) => {
  if (type === "expenses") {
    ...
    document.querySelector(".expense-top-month-value").textContent =
      Object.values(topMonth)[0];
  } else {
    document.querySelector(".income-top-month").textContent = getHumanMonth(
      Object.keys(topMonth)[0]
    );
    document.querySelector(".income-top-month-value").textContent =
      Object.values(topMonth)[0];
  }
};
...
const updateThisMonthUI = (data = [], type = "expenses") => {
  ...
  if (type === "expenses") {
    ...
    document.querySelector(".expense-this-month-value").textContent =
      Object.values(currentMonthData)[0];
  } else {
    document.querySelector(".income-this-month").textContent = getHumanMonth(
      Object.keys(currentMonthData)[0]
    );
    document.querySelector(".income-this-month-value").textContent =
      Object.values(currentMonthData)[0];
  }
};
